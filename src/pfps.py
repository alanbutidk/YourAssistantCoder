...
from notify import (
    ...
    notify_suspicious_chain,
    notify_dirty_session,
)
...
class PFPS:
    ...
    def __init__(self, project_root: str):
        ...
        self.yac_dir = self.root / ".YAC"
        self.pfps_dir = self.yac_dir / ".PFPS"
        self.skills_dir = self.yac_dir / ".SKILLS"
    ...
    def RegisterProject(
        ...
        # Copy the bundled default skill into the project's .YAC/.SKILLS dir
        instance._install_default_skill()

        # Take initial HEAD snapshot of all tracked files
        instance._snapshot_all()
        return instance

    def _install_default_skill(self):
        """Copy the bundled src/SKILLS/DEFAULT.md skill into this project's
        .YAC/.SKILLS folder so every new project starts with the default
        skill available locally (and editable per-project from there)."""
        import shutil

        bundled_skill = Path(__file__).resolve().parent / "SKILLS" / "DEFAULT.md"
        target_skill = self.skills_dir / "DEFAULT.md"
        try:
            if bundled_skill.exists():
                shutil.copyfile(bundled_skill, target_skill)
        except OSError:
            pass
    ...
    def LoadProject(cls, root: str) -> "PFPS":
        ...
        if not instance.sandbox_json.exists():
            raise FileNotFoundError(
                f"No PFPS project found at {root}. Run 'yac init' first."
            )
    ...
    def SandboxPath(self, path: str) -> Path:
        ...
        if not str(real).startswith(str(self.root)):
            raise SandboxViolation(
                f"[SANDBOX] Escape attempt blocked: {path!r} resolves outside sandbox"
            )
    ...
    def DefaultSkillContent(self) -> str:
        """Return the project's default skill text (.YAC/.SKILLS/DEFAULT.md),
        ...
        bundled_skill = Path(__file__).resolve().parent / "SKILLS" / "DEFAULT.md"
        if bundled_skill.exists():
            return bundled_skill.read_text(encoding="utf-8")
    ...
    def _validate_token(self, token: AllowToken, tool: str, filename: str):
        ...
        if not token.can_use_tool(tool):
            raise PermissionError_(
                f"[DENIED] Token level {token.level!r} cannot use {tool!r}"
            )
        if filename != "*" and not token.can_touch_file(filename):
            raise PermissionError_(
                f"[DENIED] Token scope does not include {filename!r}"
            )
    ...
    def BuildVarFile(self, filename: str):
        ...
        lines = [
            f"# YAC ReadableChunks - {filename}\n",
            f"# Generated: {time.strftime('%Y-%m-%dT%H:%M:%S')}\n\n",
        ]
        for i, (label, content) in enumerate(chunks, 1):
            # Use json.dumps for safe embedding: it produces a Python-literal-
            # compatible double-quoted string, escaping newlines, tabs,
            # backslashes, quotes, and control characters. The previous
            # hand-rolled escaping (backslash/quote only) left raw newlines
            # inside a single-line string literal, which made the generated
            # .var file a SyntaxError for virtually any real multi-line
            # chunk - breaking exec() in ReadChunkVar and, with it, every
            # read_chunk/write_chunk tool call.
            safe_label = json.dumps(label)
            safe_content = json.dumps(content)
            lines.append(f"CHUNK_{i} = ({safe_label}, {safe_content})\n")
                    ...
                    if Path(event.src_path).resolve() == filepath.resolve():
                        ...
                        if lock_p.exists():
                            ...
                            if lock_data["held_by"] == LockState.AgentHolds:
                                ...
                                with open(lock_p, "w", encoding="utf-8") as f:
                                    json.dump(
                                        {**lock_data, "held_by": LockState.UserHolds}, f
                                    )
                        ...
                        if lock_p.exists():
                            ...
                            if lock_data["held_by"] == LockState.UserHolds:
                                ...
                                sleep_duration = time.time() - (
                                    inner_self.sleep_start or time.time()
                                )
                                token.extend(sleep_duration)
                                with open(lock_p, "w", encoding="utf-8") as f:
                                    json.dump(
                                        {**lock_data, "held_by": LockState.AgentHolds},
                                        f,
                                    )
    ...
    def ReadChunk(
        self, token: AllowToken, filename: str, chunk_key: str = "CHUNK_1"
    ) -> str:
        ...
        var_path = (
            self.chunks_dir / f"_{filename.replace('/', '_').replace(chr(92), '_')}.var"
        )
        if not var_path.exists():
            self.BuildVarFile(filename)
        ...
        if chunk_key not in chunks:
            ...
            raise KeyError(
                f"Chunk {chunk_key!r} not found in {filename}. Available: {available}"
            )
    ...
    def WriteChunk(
        self, token: AllowToken, filename: str, chunk_key: str, new_content: str
    ):
