...
class AMS:
    ...
    def UseTool(self, toolname: str, token: AllowToken, **kwargs):
        ...
        if toolname not in self.ALLOWED_TOOLS:
            ...
            raise ValueError(
                f"[BLOCKED] Tool {toolname!r} is not in the allowed tool list"
            )
        ...
        # Dispatch to PFPS
        dispatch = {
            "read_chunk": lambda: self.pfps.ReadChunk(
                token, kwargs["filename"], kwargs["chunk_key"]
            ),
            "write_chunk": lambda: self.pfps.WriteChunk(
                token, kwargs["filename"], kwargs["chunk_key"], kwargs["content"]
            ),
            "append_chunk": lambda: self.pfps.AppendChunk(
                token, kwargs["filename"], kwargs["content"]
            ),
            "create_file": lambda: self.pfps.CreateFile(
                token, kwargs["filename"], kwargs.get("content", "")
            ),
            "delete_file": lambda: self.pfps.DeleteFile(
                token, kwargs.get("chunk_key", ""), kwargs["filename"]
            ),
            "list_files": lambda: self.pfps.ListFiles(
                token, kwargs.get("subpath", ".")
            ),
        }
    ...
    def SpecialUse(self, String: str, ask_confirm: bool = False) -> str:
        ...
        if ask_confirm:
            ...
            self.console.print(f"   Agent wants to execute: [cyan]{String}[/cyan]")
            answer = (
                self.console.input(
                    "   Allow? [bold green]y[/bold green]/[bold red]n[/bold red]: "
                )
                .strip()
                .lower()
            )
    ...
    def _prompt_terminate(self, filename: str):
        ...
        self.console.print(
            f"   Suspicious operation chain detected on [cyan]{filename}[/cyan]"
        )
        answer = (
            self.console.input(
                "   Terminate agent response? [bold red]y[/bold red]/[bold green]n[/bold green]: "
            )
            .strip()
            .lower()
        )
